function fetchEtAffichePays(paysChoisi) {
    let url = "http://localhost/wordpress/wp-json/wp/v2/pays?_embed&per_page=57"; // URL du serveur local
 // let url = `http://localhost:8085/4w4-2024-gr1/wp-json/wp/v2/pays?per_page=57`; // URL du serveur local
...
.then(function(data) {
    ...
    filteredData.forEach(function (article) {
        ...
        <img src="${article.acf.image ? article.acf.image : (article._embedded['wp:featuredmedia'][0].source_url ? article._embedded['wp:featuredmedia'][0].source_url : 'https://via.placeholder.com/150')}" alt="Image de la destination" style="width: 150px; height: 150px;">
        `;
